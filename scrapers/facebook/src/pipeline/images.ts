/**
 * Listing image persistence: noop (local) or DigitalOcean Spaces upload.
 *
 * Env (spaces mode):
...
export function objectKeyForImage(sourceUrl: string, body: Uint8Array): string {
  ...
  return `listings/${hash}-${randomUUID().slice(0, 8)}.${ext}`;
}
...
export async function persistListingImages(
  ...
  for (const url of sourceUrls) {
    ...
    out.push(
      await upload({ sourceUrl: url, body, contentType }),
    );
  }
  return out;
}
